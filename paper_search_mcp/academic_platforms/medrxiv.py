...
class MedRxivSearcher(PaperSource):
    ...
    def search(self, query: str, max_results: int = 10, days: int = 30) -> List[Paper]:
        """
        List medRxiv papers of a category posted within the last N days.

        This is a category listing, not a keyword search: medRxiv offers no
        ...
        medRxiv preprints use Europe PMC, which indexes them.

        Args:
            query: Category name (e.g., "cardiovascular medicine"); spaces and case are
                normalised.
        ...
        Raises:
            ValueError: The query names no medRxiv category.
        """
        ...
        for _ in range(self.MAX_SEITEN):
            ...
            if category and not papers and gesehene_kategorien:
                ...
                raise ValueError(
                    f"'{query}' ist keine medRxiv-Kategorie. medRxiv bietet keine "
                    f"Stichwortsuche, nur Kategorielisten; in den letzten {days} "
                    f"Tagen kamen diese Kategorien vor: {bekannt}. Für eine "
                    f"Stichwortsuche über medRxiv-Preprints europepmc verwenden, "
                    f"das sie indexiert."
                )
        ...
        for versuch in range(1, self.max_retries + 1):
            try:
                ...
                if versuch == self.max_retries:
                    print(f"Failed to connect to medRxiv API after "
